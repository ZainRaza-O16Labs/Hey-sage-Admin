...
export function AiRouterForm({ initial }: { initial: ParentAgentConfig | null }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-2"><Label htmlFor="parent-instructions">Instructions</Label><Textarea id="parent-instructions" rows={14} value={form.instructions} onChange={(event) => setForm({ ...form, instructions: event.target.value })} placeholder="Describe how the AI Router should coordinate configured categories and agents." /></div>
    ...
    {message ? <AiMessageBanner kind="success" onDismiss={() => setMessage(null)}>{message}</AiMessageBanner> : null}
